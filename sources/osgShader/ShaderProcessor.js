...
ShaderProcessor.prototype = {
    ...
    _convertToWebGL2: function ( strShader, isFragment ) {
        ...
        strShader = strShader.replace( /#\s*extension \s*GL_EXT_shader_texture_lod/, '// core gl2 : GL_EXT_shader_texture_lod' );
        strShader = strShader.replace( /#\s*extension \s*GL_OES_standard_derivatives/, '// core gl2 : GL_OES_standard_derivatives' );

        if ( isFragment ) {
            strShader = strShader.replace( /gl_FragColor/g, 'glFragColor_0' );
            strShader = strShader.replace( /void\s*main\s*\(/g, 'out vec4 glFragColor_0;\nvoid main(' );
        }
    ...
    processShader: function ( shader, defines, extensions, type ) {
        ...
        // if the shader starts with #version we skip the shader processing
        if ( shader.indexOf( '#version ' ) === 0 ) {
            return shader;
        }
